...
export function SolutionIconBadges({ badges }: SolutionIconBadgesProps) {
  return (
    <section className="w-full py-12 md:py-16" style={{ background: '#EDFFE4' }}>
      <Container>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 my-24">
          {badges.map((badge, idx) => (
            <div key={idx} className='flex flex-row gap-4 w-full items-center justify-center'>
              <div key={idx} className="flex flex-col items-center text-center gap-4 w-full">
              ...
              {idx !== badges.length - 1 && <div className='h-full w-0.5' style={{ backgroundColor: '#1f7a63' }}></div>}
            </div>
          ))}
        </div>
      </Container>
